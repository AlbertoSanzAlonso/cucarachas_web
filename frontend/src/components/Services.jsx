...
const Services = () => {
  ...
  return (
    <section id="servicios" className="bg-bg-light py-24 lg:py-36 relative overflow-visible">
      {/* Structural Anchor - Background blur scoped to prevent clipping */}
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-0 w-96 h-96 bg-primary-blue/5 rounded-full blur-[120px] -mr-48 -mt-48" />
      </div>

      <div className="container relative z-10 px-4">
        {/* Header Section - Refined Editorial Layout */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-20 gap-10">
          <div className="max-w-2xl text-left">
            <h2 className="text-secondary-gray mb-8 leading-tight font-black tracking-tighter">
          ...
          <div className="flex items-center gap-5 bg-white px-8 py-5 rounded-[2rem] shadow-xl border border-blue-50 h-fit self-start md:self-auto transform hover:translate-y-[-4px] transition-transform duration-300">
        ...
        {/* Species Comparison Grid - Fixed Alignment and Premium Cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {cockroachTypes.map((type) => (
            <div 
              key={type.id}
              onClick={() => setSelectedType(type)}
              className="bg-white rounded-[3.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.1)] transition-all duration-500 cursor-pointer group flex flex-col h-full ring-1 ring-gray-50 hover:ring-primary-blue/10"
            >
              ...
              <div className="flex flex-col flex-1 pl-2">
                ...
                <div className="mt-auto pt-8 border-t border-gray-50 flex items-center justify-between group-hover:border-primary-blue/10 transition-colors">
      ...
      {/* Simplified High-Reliability Modal - Direct User Pattern with Refined Styling */}
      {selectedType && (
        <div 
          className="fixed inset-0 z-[200000] flex items-center justify-center p-4 sm:p-6 lg:p-12 bg-black/95 backdrop-blur-xl animate-in fade-in duration-300"
          onClick={closeModal}
        >
          <div 
            className="relative w-full max-w-6xl bg-white rounded-[3.5rem] shadow-[0_50px_100px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col lg:flex-row z-[200001] max-h-[90vh] lg:h-[80vh] animate-in zoom-in-95 duration-500"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Close Trigger */}
            <button 
              className="absolute top-8 right-8 w-14 h-14 rounded-full bg-secondary-gray/5 text-secondary-gray hover:bg-black hover:text-white transition-all z-[200002] flex items-center justify-center shadow-lg border border-white/20 active:scale-95" 
              onClick={closeModal}
            >
              <X size={28} />
            </button>
            ...
            <div className="lg:w-3/5 p-10 lg:p-20 overflow-y-auto flex flex-col items-start text-left bg-white scrollbar-hide">
              <div className="mb-12">
                <span className="text-primary-blue/40 font-serif italic text-2xl lg:text-3xl block mb-3">{selectedType.scientificName}</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mb-16 w-full">
                <div className="p-8 bg-blue-50/30 rounded-[3rem] border border-blue-100 flex flex-col items-start transform hover:translate-y-[-4px] transition-transform duration-300">
                  <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-primary-blue mb-6 shadow-sm ring-1 ring-blue-50">
                    <Home size={28} />
                  </div>
                ...
                <div className="p-8 bg-red-50/30 rounded-[3rem] border border-red-100 flex flex-col items-start transform hover:translate-y-[-4px] transition-transform duration-300">
                  <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-red-500 mb-6 shadow-sm ring-1 ring-red-50">
                    <AlertCircle size={28} />
                  </div>
              ...
              <div className="flex flex-col sm:flex-row gap-8 items-center pt-10 border-t border-gray-100 mt-auto w-full group/footer">
                <a 
                  href="#contacto" 
                  onClick={closeModal}
                  className="btn btn-primary px-16 py-6 text-2xl font-black w-full sm:w-auto shadow-2xl hover:scale-105 active:scale-100 transition-all"
                >
                  SOL·LICITAR TRACTAMENT
                </a>
                <div className="flex items-center gap-4 ml-auto sm:ml-0 bg-bg-light px-6 py-3 rounded-full border border-gray-50">
                  <div className="w-3 h-3 rounded-full bg-emerald-500 animate-pulse shadow-[0_0_15px_#10b981]" />
                  <span className="text-[12px] font-black text-secondary-gray/50 uppercase tracking-[0.2em]">
                    RESPOSTA EN 24H
                  </span>
                </div>
              </div>
