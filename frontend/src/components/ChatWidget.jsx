  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-24 left-4 md:left-12 xl:left-[calc((100vw-1280px)/2+48px)] z-[110] w-12 h-12 md:w-16 md:h-16 flex items-center justify-center bg-primary-blue text-white rounded-full shadow-2xl hover:bg-primary-blue-hv transition-all group overflow-hidden"
        style={{ boxShadow: '0 15px 35px rgba(0, 128, 187, 0.4)' }}
      >
        <div className="flex items-center justify-center w-full h-full">
           {isOpen ? <X size={24} /> : <MessageSquare size={24} className="md:w-7 md:h-7" />}
        </div>
        {!isOpen && (
           <span className="absolute left-full ml-4 top-1/2 -translate-y-1/2 bg-white text-primary-blue px-3 py-1 rounded-lg text-[10px] font-black shadow-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity hidden md:block border border-blue-50 uppercase tracking-widest">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 left-4 md:bottom-32 md:left-12 z-[110] w-[calc(100vw-2rem)] md:w-96 h-[500px] bg-white rounded-3xl shadow-3xl overflow-hidden border border-gray-100 flex flex-col [@media(max-height:600px)_and_(orientation:landscape)]:h-[80vh] [@media(max-height:600px)_and_(orientation:landscape)]:bottom-4"
          >
            {/* Header */}
