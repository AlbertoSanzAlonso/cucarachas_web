...
- **Assistent oficina**: pestanya `ops` (`AdminOpsChat.jsx`) — xat intern (no Bio-Assistent web). Backend: `api/agents/ops/`. Historial `AdminConversation` / notes `AdminMemoryNote`. API auth `/api/ops/conversations/` i `/api/ops/notes/`. Selector de model (`GET /api/ops/models/`). Micròfon (onda → Whisper intern; TTS opcional). WhatsApp via OpenWA (`search_whatsapp_contacts`, `send_whatsapp`, env `OPENWA_*`). Email via SMTP Django (`email_status`, `send_email`, env `EMAIL_*` / `OPS_EMAIL_DRY_RUN`). **Confirmació UI**: WhatsApp/email/lead iGEO → `pending_action` + modal; l'enviament real va amb `confirm_action` (sense LLM). **RAG operatiu** (`audience=ops` a `TechnicalKnowledge`): procediments iGEO/PDI via `sync_ops_knowledge` + tool `search_ops_knowledge`; dades vives segueixen a espill/CRM (no vectors).
